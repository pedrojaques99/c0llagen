    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-6xl glass rounded-[2.5rem] overflow-hidden flex flex-col md:flex-row shadow-2xl border border-white/10"
        >
          {/* Main Player Area */}
          <div className="flex-1 flex flex-col overflow-hidden border-r border-white/5">
            <div className="p-6 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
              ...
              <button
                onClick={onClose}
                className="p-2 hover:bg-white/10 rounded-full transition-all hover:rotate-90 duration-300 md:hidden"
              >
                <X size={20} />
              </button>
            </div>
          ...
          {/* Settings Sidebar */}
          <div className="w-full md:w-[320px] bg-white/[0.01] flex flex-col">
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              ...
              <button
                onClick={onClose}
                className="hidden md:flex p-2 hover:bg-white/10 rounded-full transition-all hover:rotate-90 duration-300"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 scrollbar-hide">
              <ControlGroup label="Motion Time" icon={<Clock size={12} />}>
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center text-[10px] font-mono text-white/40">
                    <span>Duration</span>
                    <span className="text-white bg-white/10 px-2 py-0.5 rounded-lg">{durationPerSlide}s</span>
                  </div>
                  <input 
              ...
              <ControlGroup label="Playback Speed" icon={<Zap size={12} />}>
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center text-[10px] font-mono text-white/40">
                    ...
                    <span className="text-white bg-white/10 px-2 py-0.5 rounded-lg">{speed}x</span>
                  </div>
                  <input 
              ...
              <ControlGroup label="Zoom Magnitude" icon={<Maximize2 size={12} />}>
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center text-[10px] font-mono text-white/40">
                    <span>Intensity</span>
                    <span className="text-white bg-white/10 px-2 py-0.5 rounded-lg">{(zoomScale - 1).toFixed(2)}x</span>
                  </div>
                  <input 
              ...
              <ControlGroup label="Panning Amount" icon={<MoveHorizontal size={12} />}>
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center text-[10px] font-mono text-white/40">
                    <span>Range</span>
                    <span className="text-white bg-white/10 px-2 py-0.5 rounded-lg">{panAmount}%</span>
                  </div>
                  <input 
              ...
              <div className="mt-auto pt-8">
                <button 
                  ...
                  className="w-full py-3 text-[10px] uppercase tracking-widest font-bold text-white/20 hover:text-white transition-colors"
                >
                  Reset to Defaults
                </button>
              </div>
