...
export const BentoItem: React.FC<BentoItemProps> = React.memo(({
  ...
  return (
    <motion.div 
      ...
      className={`group relative aspect-square rounded-[2rem] overflow-hidden glass flex flex-col transition-all duration-500 ${isSelected ? 'ring-2 ring-ink ring-offset-4 ring-offset-bg' : ''}`}
    >
      <div className="relative flex-1 overflow-hidden cursor-zoom-in">
        ...
        <div className="absolute inset-0 bg-transparent group-hover:bg-ink/5 transition-colors pointer-events-none" />
          ...
          <AnimatePresence>
            {showPromptInput ? (
              ...
              <motion.div 
                ...
                className="glass p-4 rounded-3xl flex flex-col gap-2 w-52 shadow-2xl"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-muted">Presets</span>
                  <IconButton 
                ...
                <div className="grid grid-cols-1 gap-1">
                  {(['zoom-in', 'zoom-out', 'pan-lr', 'pan-rl', 'fade-in'] as AnimationPreset[]).map(preset => (
                    <button
                      ...
                      className="text-[10px] text-left px-4 py-2.5 rounded-xl hover:bg-ink hover:text-bg transition-all capitalize font-medium text-ink"
                    >
                      {preset.replace('-', ' ')}
                    </button>
                  ))}
              ...
              <div className="flex gap-4">
                {crop.upscaledUrl && !crop.isAnimating && !crop.videoUrl && (
                  <>
                    ...
                    <motion.button
                      ...
                      className="w-12 h-12 rounded-full bg-glass backdrop-blur-2xl text-ink border border-border hover:bg-glass-muted hover:scale-105 transition-all shadow-2xl flex items-center justify-center"
                    >
                ...
                {crop.videoUrl && (
                  <button 
                    ...
                    className="w-16 h-16 rounded-full bg-ink/10 backdrop-blur-3xl text-ink border border-border shadow-2xl scale-90 group-hover:scale-100 transition-all duration-500 flex items-center justify-center"
                  >
        ...
        {crop.upscaledUrl && !crop.videoUrl && (
          <div className="absolute bottom-4 right-4 animate-in fade-in duration-500">
            <div className="px-3 py-1.5 bg-ink text-bg text-[9px] font-bold rounded-full uppercase tracking-widest shadow-2xl">
              4K
            </div>
          </div>
        )}
      ...
      <div className="p-4 flex items-center justify-between border-t border-border bg-glass-muted">
        <span className="text-[10px] font-mono text-muted uppercase tracking-widest">#{index + 1}</span>
